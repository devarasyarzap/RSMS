...
exports.createPatientOffline = async (req, res) => {
    try {
        ...
        const { nik, name, date_of_birth, gender, address, phone } = req.body;

        // Cek duplikasi NIK
        const existingPatient = await Patient.findOne({ where: { nik } });
        if (existingPatient) {
            return res.status(400).json({ message: 'Pasien dengan NIK ini sudah terdaftar' });
        }
        ...
        const newPatient = await Patient.create({
            user_id: null, // Null karena ini daftar offline (belum punya akun login)
