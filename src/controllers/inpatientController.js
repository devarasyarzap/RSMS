...
exports.getAllInpatients = async (req, res) => {
  try {
    ...
    const admissions = await InpatientAdmission.findAll({
      where,
      include: [
        { model: Patient },
        {
        ...
        { model: Bed, include: [WardClass] },
      ],
      order: [["check_in_time", "DESC"]],
    });

    res.json({
      message: "Data rawat inap berhasil diambil",
...
exports.admitPatient = async (req, res) => {
  try {
    ...
    if (!bed || bed.status !== "available") {
      return res
        .status(400)
        .json({ message: "Bed tidak ditemukan atau sedang penuh" });
    }
    ...
    res
      .status(201)
      .json({ message: "Pasien berhasil masuk rawat inap", data: admission });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.dischargePatient = async (req, res) => {
  try {
    ...
    if (!admission || admission.status === "discharged") {
      return res
        .status(400)
        .json({ message: "Data tidak valid atau pasien sudah pulang" });
    }
